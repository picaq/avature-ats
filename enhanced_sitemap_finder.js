...
/**
 * Script to find sitemap.xml URLs for Avature career sites and extract job URLs.
 * When sitemap_index.xml is found, it lists the content of all <loc> elements
 * from sitemaps ending in careers/sitemap.xml or jobs/sitemap.xml
 */
...
async function findSitemapsForDomain(baseUrl) {
    ...
    if (check1.accessible) {
        ...
        console.log(`    Extracted ${urls.length} job URLs`);
    }
    ...
    if (robotsSitemaps) {
        for (const sitemap of robotsSitemaps) {
            if (!results.foundSitemaps.includes(sitemap)) {
                ...
                if (sitemap.endsWith('/careers/sitemap.xml') || sitemap.endsWith('/jobs/sitemap.xml')) {
    ...
    if (check3.accessible) {
        ...
        if (indexSitemaps) {
            ...
            for (const sitemap of indexSitemaps) {
                ...
                // Check if this is a careers/sitemap.xml or jobs/sitemap.xml
                if (sitemap.endsWith('/careers/sitemap.xml') || sitemap.endsWith('/jobs/sitemap.xml')) {
 ...
 */
async function main() {
    console.log(`Checking ${DOMAINS.length} domains for sitemaps and extracting job URLs...\n`);
    ...
    for (let i = 0; i < DOMAINS.length; i++) {
        ...
        if (results.foundSitemaps.length > 0) {
            console.log(`  ✓ Found ${results.foundSitemaps.length} sitemap(s), ${results.jobUrls.length} job URLs`);
        } else {
    ...
    console.log(`Total job URLs extracted: ${totalJobUrls}`);
    console.log();
    ...
    const outputContent = allResults.map(result => {
        ...
        if (result.jobUrls.length > 0) {
            content += `\n## Job URLs (${result.jobUrls.length}):\n`;
    ...
    fs.writeFileSync('job_urls.txt', allJobUrls.join('\n'));
    ...
    console.log('  - sitemaps_found.txt (organized by domain with job URLs)');
    console.log('  - sitemaps_urls_only.txt (just sitemap URLs)');
    console.log('  - job_urls.txt (all extracted job URLs)');
